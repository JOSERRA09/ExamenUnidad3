<template>
      ...
      <div class="container mt-4">
        ...
        <div v-else>
          <div v-if="series">
            ...
            <div v-if="seasons && seasons.length">
              <div class="row">
                <div class="col-md-3" v-for="season in seasons" :key="season.id" @click="loadSeasonDetails(series.id, season.season_number)" style="cursor: pointer;">
                  <div class="card">
                    <img :src="'https://image.tmdb.org/t/p/w500' + season.poster_path" :alt="season.name" class="card-img-top" />
                    <div class="card-body">
                      <p class="card-text">{{ season.name }}</p>
                      <p class="card-text">Episodios: {{ season.episode_count }}</p>
                    </div>
                  </div>
                </div>
              </div>
  ...
  const fetchSeriesDetails = async (seriesId) => {
    ...
    try {
      ...
      seasons.value = seriesResponse.data.seasons;
    } catch (error) {
  ...
  const loadSeasonDetails = (seriesId, seasonNumber) => {
    window.location.href = `/DetallesTemporada?seriesId=${seriesId}&seasonNumber=${seasonNumber}`;
  };
  </script>
